...
export const vehiculoRepository = {
    async findAll(filtros: FiltrosVehiculo = {}) {
        ...
        return vehiculos.map(v => {
            ...
            // Si no hay historial de mantenimiento, calcular el próximo intervalo
            // basándose en el kilometraje actual (no desde 0)
            let proximoMantKm: number;
            if (!ultimoMantCompletado) {
                // Nuevo vehículo sin historial: calcular el próximo múltiplo de 5000
                // Ejemplo: 7000km → próximo a 10000km
                proximoMantKm = Math.ceil(v.kilometrajeActual / INTERVALO_MANTENIMIENTO_KM) * INTERVALO_MANTENIMIENTO_KM;
                // Si está exactamente en un múltiplo, el próximo es ese + 5000
                if (proximoMantKm === v.kilometrajeActual) {
                    proximoMantKm += INTERVALO_MANTENIMIENTO_KM;
                }
            } else {
                // Vehículo con historial: calcular desde el último mantenimiento
                proximoMantKm = kmUltimoMant + INTERVALO_MANTENIMIENTO_KM;
            }
