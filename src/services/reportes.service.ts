...
export const reportesService = {
    ...
    async reporteMensualComparativo(anio: number, meses?: number[]) {
        ...
        for (const mes of mesesConsulta) {
            ...
            // FIX: Solo contar pagos REALMENTE efectuados, no montos pactados
            // El dashboard ya tiene este fix (#2), aplicamos lo mismo aquí
            const pagosChoferes = await pagosChoferRepository.sumarPagos(undefined, fechaInicio, fechaFin);

            const gastosMantenimientos = await mantenimientosRepository.sumarCostos(undefined, fechaInicio, fechaFin);

            const totalGastos = gastosViaticos + pagosChoferes + gastosMantenimientos;
