...
async function crearMantenimientosPendientes() {
    try {
        ...
        for (const vehiculo of vehiculos) {
            ...
            const ultimoMantCompletado = ultimoMant?.estado === 'COMPLETADO' ? ultimoMant : null;

            // Calcular próximo mantenimiento correctamente
            let proximoMantKm: number;
            if (!ultimoMantCompletado) {
                // Vehículo sin historial: próximo múltiplo de 5000 desde km actual
                proximoMantKm = Math.ceil(vehiculo.kilometrajeActual / INTERVALO_MANTENIMIENTO_KM) * INTERVALO_MANTENIMIENTO_KM;
                if (proximoMantKm === vehiculo.kilometrajeActual) {
                    proximoMantKm += INTERVALO_MANTENIMIENTO_KM;
                }
            } else {
                proximoMantKm = (ultimoMantCompletado.kilometrajeAlMomento || 0) + INTERVALO_MANTENIMIENTO_KM;
            }
