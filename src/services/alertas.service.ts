...
export const alertasService = {
    ...
    async obtenerAlertasMantenimiento(): Promise<AlertaMantenimiento[]> {
        ...
        for (const v of vehiculos) {
            ...
            let proximoMantenimientoKm: number;
            if (!ultimoMantCompletado) {
                // Vehículo sin historial: calcular el próximo múltiplo de 5000
                // Ejemplo: 7000km → próximo a 10000km
                proximoMantenimientoKm = Math.ceil(v.kilometrajeActual / INTERVALO_MANTENIMIENTO_KM) * INTERVALO_MANTENIMIENTO_KM;
                // Si está exactamente en un múltiplo, el próximo es ese + 5000
                if (proximoMantenimientoKm === v.kilometrajeActual) {
                    proximoMantenimientoKm += INTERVALO_MANTENIMIENTO_KM;
                }
            } else {
                proximoMantenimientoKm = (ultimoMantCompletado.kilometrajeAlMomento || 0) + INTERVALO_MANTENIMIENTO_KM;
            }
            ...
            // Ya no necesitamos el caso especial para vehículos sin historial
            // porque la lógica anterior ya lo maneja correctamente
        }
